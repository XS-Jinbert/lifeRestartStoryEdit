  ...
  methods: {
    GetAllItem() {
      ...
      if (this.axios.defaults.headers.common["token"] == null) {
        ...
        this.axios
          .get("http://localhost:8848/chooseEffect/findAll")
          .then((response) => {
    ...
    addItemRequest(unit) {
      // 发送请求
      this.axios
        .post("http://localhost:8848/chooseEffect/add", unit)
        .then((response) => {
          if (this.result(response)) {
            this.GetAllItem();
            this.GetSearchItems();
    ...
    updateItemRequest(unit) {
      // 发送请求
      this.axios
        .post("http://localhost:8848/chooseEffect/update", unit)
        .then((response) => {
          if (this.result(response)) {
            this.GetAllItem();
            this.GetSearchItems();
    ...
    deleteItemRequest(unit) {
      // 发送请求
      this.axios
        .delete("http://localhost:8848/chooseEffect/delete", { data: unit })
        .then((response) => {
          if (this.result(response)) {
            this.GetAllItem();
            this.GetSearchItems();
    ...
    searchItemsRequest(unit) {
      ...
      this.axios
        .post("http://localhost:8848/chooseEffect/select", unit)
        .then((response) => {
          if (this.result(response)) {
