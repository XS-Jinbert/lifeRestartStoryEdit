  ...
  methods: {
    GetAllItem() {
      ...
      if (this.axios.defaults.headers.common["token"] == null) {
        ...
        this.axios
          .get("http://localhost:8848/eventBranch/findAll")
          .then((response) => {
    ...
    addItemRequest(unit) {
      // 发送请求
      this.axios
        .post("http://localhost:8848/eventBranch/add", unit)
        .then((response) => {
          if (this.result(response)) {
            this.GetAllItem();
            this.GetSearchItems();
    ...
    updateItemRequest(unit) {
      // 发送请求
      this.axios
        .post("http://localhost:8848/eventBranch/update", unit)
        .then((response) => {
          if (this.result(response)) {
            this.GetAllItem();
            this.GetSearchItems();
    ...
    deleteItemRequest(unit) {
      // 发送请求
      this.axios
        .delete("http://localhost:8848/eventBranch/delete", { data: unit })
        .then((response) => {
          if (this.result(response)) {
            this.GetAllItem();
            this.GetSearchItems();
    ...
    searchItemsRequest(unit) {
      ...
      this.axios
        .post("http://localhost:8848/eventBranch/select", unit)
        .then((response) => {
          if (this.result(response)) {
